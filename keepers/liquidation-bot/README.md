...
- Fee logs: `job_plan`, `job_skip`, `tx_sent`, `tx_confirmed` include `fee.mode/source/known/maxFeePerGas/maxPriorityFeePerGas/gasPrice`. Modes align with submission: `eip1559` for `maxFeePerGas`, `legacy` for `gasPrice`, `unknown` when fee resolution failed and no cap is enforced.
